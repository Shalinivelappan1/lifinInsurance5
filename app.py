...
with st.sidebar.expander("ℹ️ What is the discount rate?"):
    st.markdown("""
The discount rate is your **opportunity cost of capital**:
- What you believe you can earn elsewhere (equity, business, etc.)
- It reflects **time preference + risk preference**
- A higher discount rate penalizes **long-duration** cash flows more severely.
""")
...
**Disability / Income Replacement:** If disability occurs before age 60, pays a living-income style benefit (modeled approximately).  
...
def insurance_npv(age, term, coverage, premium, discount_rate, gender, smoker, health,
    ...
    for t in range(1, effective_term + 1):
        ...
        # Critical illness: 10% chance, pays 30% of cover (teaching simplification)
        if critical:
            expected_payout += survival * 0.10 * 0.3 * coverage

        # Disability income: only before age 60, pays 20% of cover (teaching simplification)
        if disability and (age + t) < 60:
            expected_payout += survival * 0.05 * 0.2 * coverage

        pv_benefit += expected_payout / ((1 + r) ** t)

        # Waiver of premium: if triggered before age 55 and after some years, future premiums drop to zero (simplified expectation)
...
- Usually **negative** for good insurance because the insurer must cover costs, capital, and profit.

**Utility (Protection View):**
- A proxy for **how much financial ruin is avoided**.
- High cover with manageable cost = high protection value.
""")
...
# =====================================================
# Decision Summary (on screen + for download)
# =====================================================
summary_text = f"""
DECISION SUMMARY
----------------
...
NPV: {npvA:,.0f} | {npvB:,.0f}
Utility Score: {utilA:,.1f} | {utilB:,.1f}
Cover: {coverA:,.0f} | {coverB:,.0f}
Premium: {premA:,.0f} | {premB:,.0f}

Riders:
Accidental: {accA} | {accB}
Critical Illness: {ciA} | {ciB}
Disability: {disA} | {disB}
Waiver of Premium: {wopA} | {wopB}
"""
...
with st.expander("How to think about this"):
    st.markdown("""
...
- This is fundamentally a **duration** problem, not just a cost problem.
""")
...
- Steeper curves indicate **higher sensitivity to mispricing**.
